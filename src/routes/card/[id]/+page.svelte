<script lang="ts">
	...
	let marketPrice = $derived.by(() => {
		if (!card.tcgplayer?.prices) return null;
		const first = Object.entries(card.tcgplayer.prices)[0];
		if (!first) return null;
		return { variant: first[0], ...first[1] };
	});
	...
	// Flatten evolution chain for display
...
<div class="space-y-6">
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
		...
		<div class="flex items-start justify-center lg:col-span-1">
			<div class="sticky top-8 w-full max-w-sm">
				<img
					...
					alt={card.name}
					class="w-full rounded-xl shadow-2xl shadow-vault-accent/10"
				/>
			</div>
		</div>
		...
		<div class="space-y-6 lg:col-span-2">
						...
						<div>
							...
							<div class="mt-1 flex gap-1.5">
								{#each card.types as type}
									<span class="rounded-full px-2.5 py-0.5 text-xs font-semibold text-white {typeColor(type)}">
										{type}
									</span>
								{/each}
							</div>
				...
				{#if card.subtypes?.length}
					<div class="mt-4 flex flex-wrap gap-2">
						{#each card.subtypes as subtype}
							<span class="rounded-lg border border-vault-border px-2.5 py-1 text-xs text-vault-text-muted">
								{subtype}
							</span>
						{/each}
					</div>
									...
									<div class="flex items-center gap-3">
										<div class="flex gap-0.5">
											{#each attack.cost as costType}
												<span class="inline-block h-5 w-5 rounded-full text-center text-[10px] font-bold leading-5 text-white {typeColor(costType)}">
													{costType[0]}
												</span>
											{/each}
										</div>
				...
				<div class="rounded-xl border border-vault-border bg-vault-surface p-6">
					<div class="grid grid-cols-3 gap-4 text-center">
							...
							{#if card.weaknesses?.length}
								{#each card.weaknesses as w}
									<p class="mt-1 text-white">
										<span class="rounded-full px-2 py-0.5 text-xs font-bold text-white {typeColor(w.type)}">{w.type}</span>
										<span class="ml-1 text-vault-red">{w.value}</span>
									</p>
								{/each}
							{:else}
								<p class="mt-1 text-vault-text-muted">None</p>
							{/if}
						</div>
						<div>
							...
							{#if card.resistances?.length}
								{#each card.resistances as r}
									<p class="mt-1 text-white">
										<span class="rounded-full px-2 py-0.5 text-xs font-bold text-white {typeColor(r.type)}">{r.type}</span>
										<span class="ml-1 text-vault-green">{r.value}</span>
									</p>
								{/each}
							{:else}
								<p class="mt-1 text-vault-text-muted">None</p>
						...
						<div>
							...
							{:else}
								<p class="mt-1 text-vault-text-muted">0</p>
							{/if}
						</div>
					</div>
				</div>
			...
			<!-- Market Prices (TCGPlayer from card data) -->
					...
					<div class="flex items-center justify-between">
						...
						{#if card.tcgplayer.url}
							<a
								...
								class="text-sm text-vault-accent hover:underline"
							>
								View on TCGPlayer
							</a>
						{/if}
								...
								<div class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
									{#if prices.low != null}
										<div>
											...
											<p class="font-semibold text-white">${prices.low.toFixed(2)}</p>
										</div>
									{/if}
									{#if prices.mid != null}
										<div>
											<span class="text-vault-text-muted">Mid</span>
											<p class="font-semibold text-white">${prices.mid.toFixed(2)}</p>
										</div>
									{/if}
									{#if prices.high != null}
										<div>
											<span class="text-vault-text-muted">High</span>
											<p class="font-semibold text-white">${prices.high.toFixed(2)}</p>
										</div>
									{/if}
									{#if prices.market != null}
										<div>
											<span class="text-vault-text-muted">Market</span>
											<p class="font-bold text-vault-green">${prices.market.toFixed(2)}</p>
										</div>
									{/if}
								</div>
							</div>
				...
				<div class="rounded-xl border border-vault-border bg-vault-surface p-6">
					...
					<div class="mt-4 space-y-4">
						<div class="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
							<div>
								<span class="text-vault-text-muted">National Dex #</span>
								<p class="font-medium text-white">#{pokedexData.id}</p>
							</div>
							<div>
								<span class="text-vault-text-muted">Species</span>
								<p class="font-medium text-white">{pokedexData.genus}</p>
							</div>
							<div>
								<span class="text-vault-text-muted">Types</span>
								<div class="mt-0.5 flex gap-1">
									{#each pokedexData.types as type}
										<span class="capitalize text-white">{type}</span>
									{/each}
								</div>
							</div>
							<div>
								<span class="text-vault-text-muted">Height</span>
								<p class="font-medium text-white">{(pokedexData.height / 10).toFixed(1)}m</p>
							</div>
							<div>
								<span class="text-vault-text-muted">Weight</span>
								<p class="font-medium text-white">{(pokedexData.weight / 10).toFixed(1)}kg</p>
							</div>
						</div>
				...
				<div class="rounded-xl border border-vault-border bg-vault-surface p-6">
					...
					<div class="mt-4 flex flex-wrap items-center gap-3">
						{#each evolutions as evo, i}
							{#if i > 0}
								<svg class="h-5 w-5 text-vault-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			<div class="flex gap-3">
				<button
					...
					class="rounded-lg bg-vault-accent px-6 py-2.5 text-sm font-medium text-white transition-colors hover:bg-vault-accent-hover disabled:opacity-50"
				>
					{#if addedToCollection}
						Added!
					{:else if actionLoading === 'collection'}
						Adding...
					{:else}
						Add to Collection
					{/if}
				...
				<button
					...
					class="rounded-lg border border-vault-border px-6 py-2.5 text-sm font-medium text-vault-text transition-colors hover:bg-vault-surface-hover disabled:opacity-50"
				>
					{#if addedToWatchlist}
						Watching!
					{:else if actionLoading === 'watchlist'}
						Adding...
					{:else}
						Add to Watchlist
					{/if}
