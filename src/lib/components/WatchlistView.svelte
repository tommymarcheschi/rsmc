<script lang="ts">
	...
	import type { WatchlistValuation } from '$services/watchlist-load';
	...
	let { entries, cardCache, valuationByEntry, triggeredCount }: Props = $props();

	// Triggered entries rise to the top. Within each group, keep the server's
...
<div class="space-y-3">
	<p class="text-sm text-vault-text-muted" data-testid="watchlist-summary">
		{entries.length} card{entries.length !== 1 ? 's' : ''} tracked
		{#if triggeredCount > 0}
			· <span class="font-medium text-vault-green">{triggeredCount} triggered</span>
		{:else}
			· triggered alerts rise to the top
		{/if}
	</p>
		...
		<div class="rounded-2xl border border-vault-border bg-vault-surface">
			<div class="divide-y divide-vault-border">
				{#each sortedEntries as entry (entry.id)}
					...
					<div
						class="flex flex-wrap items-center gap-3 px-3 py-3 sm:flex-nowrap sm:gap-4 sm:px-6 sm:py-4 {val?.triggered
							? 'bg-vault-green/5'
							: ''}"
					>
						{#if card}
						...
						{:else}
							<div
								class="flex h-20 w-14 flex-shrink-0 items-center justify-center rounded-lg bg-vault-bg text-xs text-vault-text-muted"
							>
								...
							</div>
						{/if}
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								{#if card}
									<a
										href="/card/{card.id}"
										class="font-medium text-white hover:text-vault-purple">{card.name}</a
									>
								{:else}
								...
								{#if val?.triggered}
									<span
										class="rounded-full bg-vault-green/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-vault-green"
										>Triggered</span
									>
								{/if}
							</div>
						...
						<div class="flex flex-shrink-0 items-center gap-2 sm:gap-3">
							<div class="flex items-center gap-1">
								<label for="target-{entry.id}" class="text-[10px] uppercase text-vault-text-muted"
									>Target</label
								>
								<input
									id="target-{entry.id}"
									...
									placeholder="—"
									onchange={(e) =>
										updateTargetPrice(entry.id, (e.currentTarget as HTMLInputElement).value)}
							...
							<button
								...
								class="rounded-xl p-2.5 transition-colors {entry.alert_enabled
		...
		<div class="rounded-2xl border border-vault-border bg-vault-surface">
			<div class="flex items-center justify-center py-16 text-vault-text-muted">
				<div class="text-center">
					...
					<a
						href="/browse"
						class="btn-press mt-4 inline-block rounded-xl bg-gradient-to-r from-vault-accent to-vault-accent-hover px-4 py-2 text-sm font-medium text-vault-bg shadow-lg shadow-vault-accent/20 transition-all hover:shadow-vault-accent/40"
					>
						Browse Cards
					</a>
				</div>
