<script lang="ts">
  ...
  function backToAuction() {
    // Implement the logic to redirect back to the auction
    // For example, you might use `window.location.href` or Svelte's `goto` if you're using routing
    window.location.href = '/auctions'; // Replace with your actual auction page URL
  }
</script>
...
<dialog open class="modal">
  <div class="modal-box w-full md:max-w-xl px-5 py-2 md:px-10 md:py-4 border-2 border-black bg-black rounded-none items-center border-box font-incon flex flex-col z-[100] ">
    ...
    {:else}
      <form method="dialog" class="text-white font-incon">
        <button class="link hover:no-underline" on:click={backToAuction}>Back to Auction</button>
      </form>
    {/if}
  </div>
</dialog>
